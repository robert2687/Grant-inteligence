...
// Performance: Memoize GrantCard sub-component to prevent unnecessary Virtual DOM diffing during parent state updates (e.g. typing in filter inputs)
// Consumes useLanguage directly to avoid unmemoized function prop dependency issues.
const GrantCard = React.memo(({ grant }: { grant: Grant }) => {
  const { t, locale } = useLanguage();

  // Performance: Memoize date formatting to eliminate repetitive string parsing and formatting on every re-render
