...
export default function ProjectManager() {
  ...
  // Performance: Memoize active project lookup to avoid O(N) search on every render
  const activeProject = useMemo(() =>
    projects.find(p => p.id === activeProjectId) || null,
    [projects, activeProjectId]
  );

  // Performance: Use lazy initialization for state to avoid redundant object creation
  // and crypto.randomUUID() calls on every render cycle.
  ...
  // Performance: Pass the project object directly to avoid an unnecessary O(N) .find() lookup
